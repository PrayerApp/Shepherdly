...
import {
  ...
  type GtMapping,
  type MappingDraft,
  type MappingLayerPerson,
  type LayerInclusion,
  type TreeConnection,
  type ShepherdOverRule,
  type MetricBucket,
  type GroupLite,
  type TeamLite,
  type Card,
...
import { ShepherdOverModal } from './ShepherdOverModal'
...
export default function ShepherdTreeV2() {
  ...
  // "Shepherd Over" modal: opened from the connect-mode action bar when
  // exactly one card is selected. Lets you bulk-connect them as shepherd
  // over groups/teams/types/layers.
  const [shepherdOverOpen, setShepherdOverOpen] = useState(false)
  const [shepherdOverParent, setShepherdOverParent] = useState<{ personId: string; layerId: string } | null>(null)
  const [shepherdOverBusy, setShepherdOverBusy] = useState(false)
  ...
  // Card type is imported from ./types — same shape, shared with the
  // ShepherdOver modal so it can render the same render units.
  ...
  // bottom-up (parent centred over children), then parks any unconnected
  // cards to the right of their own layer. Implementation lives in
  // ./layout — pure function takes the resolved card universe and
  // returns x-positions plus co-leader clusters.
  ...
  // These two helpers resolve a tree connection's parent/child to the
  // appropriate cardKey on each side. They're used both by the layout
  // engine and by the rendering code (edges, hover handling, etc.) so
  // they live in main rather than in ./layout. The layout engine has
  // its own internal copies — they're identical and small enough that
  // duplication is cheaper than threading them through.
  ...
  const layout = computeTreeLayout({ connections, peopleByLayer, allLayers, sortedLayers })
  ...
  return (
    <div style={{ maxWidth: '100%', overflowX: 'hidden' }}>
      ...
      {/* ═══════════════════════════════════════════════════════════
          MODAL: Shepherd Over — bulk-connect from one parent
         ═══════════════════════════════════════════════════════════ */}
